    ...
    Stack unit
      ...
      (ensures fun h0 () h1 ->
        ...
        (let old_cv = Seq.index (B.as_seq h0 cv_state) 0 in
         ...
         let old_s = { cv = old_cv; prev_cd = old_prev } in
         let (new_s, out) = ctd_step old_s cd ld pv in
         Seq.index (B.as_seq h1 cv_state) 0 = new_s.cv /\
         Seq.index (B.as_seq h1 prev_state) 0 = (if new_s.prev_cd then 1uy else 0uy) /\
         Seq.index (B.as_seq h1 q_out) 0 = (if out.q then 1uy else 0uy) /\
         Seq.index (B.as_seq h1 cv_out) 0 = out.cv_out))

let ctd_step_c cv_state prev_state cd ld pv q_out cv_out =
  let old_cv = cv_state.(0ul) in
  let old_prev = prev_state.(0ul) <> 0uy in
  let old_s : ctd_state = { cv = old_cv; prev_cd = old_prev } in
  let result = ctd_step old_s cd ld pv in
  let new_s = fst result in
  let out = snd result in
  cv_state.(0ul) <- new_s.cv;
  prev_state.(0ul) <- (if new_s.prev_cd then 1uy else 0uy);
