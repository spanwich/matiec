(**
 * LowPLC.TP.Impl — Low* (stateful) implementation of TP pulse timer
 *
 * Wraps the pure tp_step specification from LowPLC.TP with mutable
 * buffer operations suitable for KaRaMeL extraction to C.
 *)
module LowPLC.TP.Impl
...
let tp_phase_to_u8 (p: tp_phase) : U8.t =
  match p with
  | TP_IDLE   -> 0uy
  | TP_TIMING -> 1uy
  | TP_DONE   -> 2uy

let u8_to_tp_phase (v: U8.t{U8.v v <= 2}) : tp_phase =
  if v = 0uy then TP_IDLE
  else if v = 1uy then TP_TIMING
  else TP_DONE
...
val tp_step_c:
    ...
    Stack unit
      ...
      (ensures fun h0 () h1 ->
        ...
        (let old_phase = u8_to_tp_phase (Seq.index (B.as_seq h0 phase_buf) 0) in
         ...
         let (new_s, out) = tp_step old_s in_val pt_ticks cur_tick in
         Seq.index (B.as_seq h1 phase_buf) 0 = tp_phase_to_u8 new_s.phase /\
...
let tp_step_c phase_buf prev_in_buf start_buf in_val pt_ticks cur_tick q_out et_out =
  let phase_v = phase_buf.(0ul) in
  let prev_v = prev_in_buf.(0ul) <> 0uy in
  let start_v = start_buf.(0ul) in
  let old_s : tp_state = { phase = u8_to_tp_phase phase_v; prev_in = prev_v; start_tick = start_v } in
  let result = tp_step old_s in_val pt_ticks cur_tick in
  let new_s = fst result in
  let out = snd result in
  phase_buf.(0ul) <- tp_phase_to_u8 new_s.phase;
